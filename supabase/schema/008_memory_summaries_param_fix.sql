-- Fix: allow non-UUID room ids by changing RPC param type to text
-- Reason: App uses Firestore-style room ids (non-UUID). Passing them to a UUID-typed param caused 22P02.

-- 1) Drop previous function overload that used uuid for p_room_id
...
-- 2) Re-create function with p_room_id as text (keeping the same name so PostgREST named args work)
...
-- 3) Nudge PostgREST to reload schema
do $$
begin
