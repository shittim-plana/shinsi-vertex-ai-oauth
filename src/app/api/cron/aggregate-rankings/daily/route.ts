// /src/app/api/cron/aggregate-rankings/daily/route.ts
...
import { RANKINGS_DAILY_COLLECTION } from '@/firebase/collections';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { from, to, key } = getKSTDayRange(yesterday);

    // Aggregate rankings for the day
    const doc: RankingDoc = await aggregateRankings(from, to, 'daily');

    // Upsert into Firestore (Admin)
    const db = getFirestore(adminApp);
    const docId = doc.metadata.periodKey;
    await db.collection(RANKINGS_DAILY_COLLECTION).doc(docId).set({
    ...
    console.log('[cron] aggregate-rankings/daily', {
      periodKey: docId,
    ...
    console.error('[cron] aggregate-rankings/daily error', err?.message || err);
